...
from bsm.bayesian_regression import DeterministicSimFSVGDEnsemble
...
model = BNNStatisticalModel(
    ...
    num_particles=10,
    features=[64, 64, 64],
    bnn_type=DeterministicSimFSVGDEnsemble,
    train_share=0.6,
...
statistical_model_state = model.update(
    stats_model_state=init_stats_model_state, data=data
)
...
def test_statistical_model_state_of_prediction():
    ...
    assert preds.statistical_model_state.model_state.calibration_alpha.shape == (
        output_dim,
    )
