from bsm.sims.simulators import RaceCarSim
import jax.numpy as jnp
import jax
from matplotlib import pyplot as plt
from bsm.sims.envs import RCCarSimEnv
from bsm.sims.util import decode_angles, encode_angles
...
INIT_STATE = jnp.array(
    [
        ...
        [-1, -0.5, 0.0, 1.0, 0.5, 0.5, 1.0],
        [0.5, -1.5, 0.0, -2.0, -0.5, -0.5, 1.0],
    ]
)

ACTIONS = [
    lambda t: jnp.array([-1 * jnp.sin(2 * t), 0.8 / (t + 1)]),
...
for k in range(3):
    for j, sim in enumerate([sim_lf, sim_hf]):
        ...
        axes[k][j].set_xlim(-1, 1.0)
        axes[k][j].set_ylim(-2, 1.0)

        env = RCCarSimEnv(
            encode_angle=True, use_obs_noise=False, use_tire_model=bool(j)
        )
        obs = env.reset()
