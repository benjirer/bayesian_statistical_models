...
from bsm.sims.util import plot_rc_trajectory
import time
import jax
import os
...
def generate_action_trajectory(key, num_steps=100, length_scale: float = 0.5):
    t = jnp.arange(num_steps) / 30.0
    K = tfp.math.psd_kernels.ExponentiatedQuadratic(length_scale=length_scale).matrix(
        t[:, None], t[:, None]
...
for i, key in enumerate(jax.random.split(key, 10)):
    key1, key2 = jax.random.split(key)
    actions = generate_action_trajectory(key1, length_scale=1.0, num_steps=600)
    s0 = env.reset(key2)
    traj = simulate_traj(
        s0,
        actions,
