...
from bsm.sims.dynamics_models import (
    Pendulum,
    PendulumParams,
    RaceCar,
    CarParams,
...
class SergioSim(FunctionSimulator):
    ...
    def __init__(self, n_genes: int = 20, n_cells: int = 20, use_hf: bool = False):
        FunctionSimulator.__init__(
            self, input_size=n_genes * n_cells, output_size=n_genes * n_cells
        )
    ...
    def normalization_stats(self) -> Dict[str, jnp.ndarray]:
        ...
        stats = {
            "x_mean": jnp.ones(self.input_size) * (self.state_ub + self.state_lb) / 2,
            "x_std": jnp.ones(self.input_size)
            * (self.state_ub - self.state_lb) ** 2
            / 12,
