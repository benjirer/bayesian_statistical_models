...
class SimPriorDeterministicEnsemble(DeterministicEnsemble):
    ...
    def eval_loss(
        ...
        predicted_outputs, predicted_stds = apply_ensemble(
        ...
        negative_log_likelihood = super()._neg_log_posterior(
    ...
    def evaluate_model(
        self, vmapped_params: PyTree, eval_data: Data, data_stats: DataStats
    ) -> OrderedDict:
        eval_nll, eval_mse = self.eval_loss(
            vmapped_params, eval_data.inputs, eval_data.outputs, data_stats
        )
    ...
    def fsim_samples(
        ...
        f_prior_normalized = jax.vmap(
            jax.vmap(lambda y: self.normalizer._normalize_y(y, data_stats.outputs))
        )(f_prior)
    ...
    def get_score_per_dim(self, fsamples, predictions, eps=1e-4):
        ...
        return prior_gp_approx.log_prob(predictions) / (
            self.num_measurement_points + self.batch_size
        )
    ...
    def _neg_log_posterior(
        ...
        return (
            ...
            - (1.0 / num_data_points) * prior_logprob.mean()
        )
    ...
    def loss(
        ...
        neg_log_posterior = jax.vmap(
            self._neg_log_posterior, in_axes=(0, 0, None, None, None, None, None)
        )
    ...
    def _train_model(
        ...
        train_data = Data(
            inputs=np.asarray(train_data.inputs), outputs=np.asarray(train_data.outputs)
        )
        eval_data = Data(
            inputs=np.asarray(eval_data.inputs), outputs=np.asarray(eval_data.outputs)
        )
        ...
        for train_step in range(num_training_steps):
            ...
            opt_state, vmapped_params, statistics = self.step_jit(
                opt_state,
                vmapped_params,
                data_batch.inputs,
                data_batch.outputs,
            ...
            if train_step % self.evaluation_frequency == 0:
                ...
                eval_data_batch = self.sample_batch(
                    eval_data, self.eval_batch_size, eval_rng
                )
                eval_statistics = self.evaluate_model(
                    vmapped_params=vmapped_params,
                    eval_data=eval_data_batch,
        ...
        new_model_state = BNNState(
            data_stats=data_stats,
            vmapped_params=final_params,
            calibration_alpha=calibrate_alpha,
        )
        return new_model_state


class SimPriorProbabilisticEnsemble(ProbabilisticEnsemble):
    def __init__(
        self,
        prior_h: float = 1.0,
        function_simulator: FunctionSimulator = SinusoidsSim(),
    ...
    def fsim_samples(
        ...
        f_prior = function_simulator.sample_function_vals(
            x=data_stacked, num_samples=self.num_f_samples, rng_key=key
        )
        # vmap over measurement points and then over samples
        f_prior_normalized = jax.vmap(
    ...
    def get_score_per_dim(self, fsamples, predictions, eps=1e-4):
        ...
        log_prob = prior_gp_approx.log_prob(predictions)

        # TODO: Normalization wrt the number of points in the prior helps in training.
        #  -> Should check if this is generally necessary
        return log_prob / (self.num_measurement_points + self.batch_size)
    ...
    def prior_log_prob_gp_approx(
        ...
        # calculate prior
        f_samples_mu = self.fsim_samples(
            data_stacked, key, data_stats, self.function_simulator
        )
        f_samples_sigma = self.fsim_samples(
            data_stacked,
            key,
            data_stats,
            GaussianProcessSim(input_size=self.input_dim, output_size=self.output_dim),
        )

        # calculate log prob per dim
        log_prob_per_dim_mu = jax.vmap(
            self.get_score_per_dim, in_axes=(2, 1), out_axes=0
        )(f_samples_mu, mu)

        log_prob_per_dim_sigma = jax.vmap(
            self.get_score_per_dim, in_axes=(2, 1), out_axes=0
        )(f_samples_sigma, nn.softplus(sigma))

        log_prob = log_prob_per_dim_mu.sum(0) + log_prob_per_dim_sigma.sum(0)
        return log_prob
    ...
    def _neg_log_posterior(
        ...
        prior_logprob = self.prior_log_prob_gp_approx(
            mu, sigma, data_stacked, key, data_stats
        )
    ...
    def loss(
        ...
        # vmap over the ensemble axis
        neg_log_posterior = jax.vmap(
            self._neg_log_posterior, in_axes=(0, None, None, None, None, None)
        )
        negative_log_posterior = neg_log_posterior(
            predictions,
            data_stacked,
...
if __name__ == "__main__":
    ...
    logging_wandb = False
    output_dir = "plts_sim_det_new_withstd"
    input_dim = 1
    output_dim = 1
    ...
    num_particles = 10
    model = SimPriorProbabilisticEnsemble(
        input_dim=input_dim,
    ...
    model_state = model.fit_model(
        data=data, num_training_steps=100_000, model_state=model_state
    )
